...
Provides an `Asset` facade that prefers registered providers via
...
This file intentionally keeps the implementation small and well-tested so it can
be safely cherry-picked into focused refactor branches.
"""
...
class Asset:
    ...
    - Delegates to a registered provider when available (AssetRegistry).
    - Otherwise falls back to a lightweight yfinance-backed implementation.
    """
