...
def calculate_yearly_cash_flow(transactions: List[str], start_date: date, end_date: date) -> Dict[int, float]:
...
def run_strategy_backtest(
    ...
    result = {
        ...
        "price_change_pct": ((summary["end_price"] - summary["start_price"]) / summary["start_price"] * 100) if summary["start_price"] > 0 else 0.0,
        ...
        "transactions_per_year": (len(transactions) - 1) / summary["years"] if summary["years"] > 0 else 0.0,
    ...
    print(f"  Transactions: {result['num_transactions']} ({result['transactions_per_year']:.1f}/year)")
    print(f"  Final Value: ${result['total']:,.2f}")
    print(f"  Bank Balance: ${result['bank']:,.2f}")
    print(f"\nCash Flow Analysis:")
    print(f"  Positive Cash Years: {result['positive_cash_years']} / {cash_flow_stats['total_years']}")
...
def calculate_withdrawal_impact(
    ...
    annual_withdrawal: float = 50000.0
) -> List[Dict[str, Any]]:
    ...
    for r in results:
        ...
        r["withdrawal_coverage_pct"] = (total_cash_generated / total_withdrawals_needed * 100) if total_withdrawals_needed > 0 else 0.0
...
def print_comparison_summary(results: List[Dict[str, Any]]) -> None:
    ...
    print(f"{'Strategy':<30} {'Ann. Return':<12} {'Txns/Yr':<10} {'Final Value':<15} {'Final Shares':<12}")
    ...
    for r in results:
        ...
        print(f"{strategy:<30} {ann_return:<12} {txns_per_year:<10} {final_value:<15} {final_shares:<12}")
    ...
    for r in results:
        ...
        positive_years = f"{r['positive_cash_years']}/{r['positive_cash_years'] + r['negative_cash_years']}"
    ...
    print(f"{'Strategy':<30} {'Cash Generated':<18} {'Needed':<15} {'Coverage':<12} {'Surplus/Deficit':<18}")
    ...
    if baseline:
        ...
        if sd8_full and "error" not in sd8_full:
            ...
            print(f"   - Transactions: {sd8_full['transactions_per_year']:.1f}/year (mix LTCG/STCG)")
            print()
