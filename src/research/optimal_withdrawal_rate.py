...
def find_optimal_withdrawal_rate(
    ...
    print(f"Testing {len(test_rates)} withdrawal rates from {min_rate*100:.0f}% to {max_rate*100:.0f}%")
...
def print_results(
    results: List[WithdrawalRateResult],
    ticker: str,
    top_n: int = 10
):
    ...
    print("-" * 80)
    print(f"{'Rank':<6} {'Rate':<8} {'Mean Bank':<12} {'Std Bank':<12} {'Margin %':<10} {'Balance Score':<15}")
    print("-" * 80)
    
    for i, result in enumerate(results[:top_n], 1):
        print(f"{i:<6} {result.withdrawal_rate*100:>6.1f}%  "
