...
def analyze_profit_sharing_spectrum(
    ...
    print(
        f"Profit Sharing Range: {profit_sharing_min:.0%} to {profit_sharing_max:.0%} (step {profit_sharing_step:.0%})"
    )
    print(f"{'='*70}\n")
    ...
    first_price = (
        float(df.iloc[0]["Close"].iloc[0])
        if hasattr(df.iloc[0]["Close"], "iloc")
        else float(df.iloc[0]["Close"])
    )
    initial_qty = int(initial_value / first_price)
    print(
        f"  Initial: {initial_qty} shares @ ${first_price:.2f} = ${initial_qty * first_price:,.2f}\n"
    )

    # Generate profit sharing ratios
    ratios = np.arange(
        profit_sharing_min, profit_sharing_max + profit_sharing_step / 2, profit_sharing_step
    )
...
def plot_composition_over_time(results: Dict, ticker: str):
    ...
    fig.suptitle(
        f"{ticker} Holdings Composition: Profit Sharing Spectrum", fontsize=16, fontweight="bold"
    )
...
def create_summary_table(results: Dict, ticker: str):
    ...
    print(f"{'='*100}")
    print(
        f"{'PS%':>6} | {'Final Value':>12} | {'Return':>8} | {'Shares':>8} | {'Cash':>12} | {'Util%':>6} | {'ROD%':>7}"
    )
    print(f"{'-'*100}")
    ...
    for ps_ratio in sorted(results.keys()):
        ...
        print(
            f"{ps_ratio:+5.0%} | ${final_value:11,.0f} | {total_return:+7.1%} | "
    ...
    print(
        f"  • Highest Return: {best_ps:+.0%} profit sharing → {results[best_ps]['total_return']:+.1%}"
    )

    # Find the highest final value
    best_value_ps = max(results.keys(), key=lambda ps: results[ps]["end_value"])
    print(
        f"  • Highest Final Value: {best_value_ps:+.0%} profit sharing → ${results[best_value_ps]['end_value']:,.0f}"
    )

    # Find most shares accumulated
    most_shares_ps = max(results.keys(), key=lambda ps: results[ps]["holdings"])
    print(
        f"  • Most Shares: {most_shares_ps:+.0%} profit sharing → {results[most_shares_ps]['holdings']:.0f} shares"
    )

    # Find most cash accumulated
    most_cash_ps = max(results.keys(), key=lambda ps: results[ps]["bank"])
    print(
        f"  • Most Cash: {most_cash_ps:+.0%} profit sharing → ${results[most_cash_ps]['bank']:,.0f}"
    )

    # Find highest capital efficiency
    best_efficiency_ps = max(
        results.keys(), key=lambda ps: results[ps].get("return_on_deployed_capital", 0)
    )
    print(
        f"  • Best Capital Efficiency: {best_efficiency_ps:+.0%} profit sharing → "
