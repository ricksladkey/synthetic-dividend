...
def run_portfolio_experiment() -> Dict:
    ...
    for i, ticker in enumerate(TICKERS):
        ...
        print(f"{ticker:<10} {ALLOCATIONS[ticker]*100:>5.0f}%        "
    ...
    print("-"*80)
    print(f"{'VOO Baseline':<25} ${baseline_result['final_value']:>18,.0f} {baseline_result['total_return']:>+13.2f}% {baseline_result['annualized_return']:>+13.2f}%")
    for strategy in strategies:
        name = f"Portfolio {strategy.replace('-', ' ').title()}"
        agg = aggregated[strategy]
        print(f"{name:<25} ${agg['final_value']:>18,.0f} {agg['total_return']:>+13.2f}% {agg['annualized_return']:>+13.2f}%")
