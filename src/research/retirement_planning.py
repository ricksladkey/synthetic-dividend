...
from src.models.retirement_backtest import run_retirement_backtest, compare_withdrawal_strategies
...
def run_single_asset_retirement_test(
    ...
    for strategy_name, algo_name in STRATEGIES.items():
        ...
        print(f"    {survived} Final: ${summary['final_value']:,.0f}, Withdrawn: ${summary['total_withdrawn']:,.0f}")
...
def run_mstr_unrealized_alpha_analysis(
    ...
    _, summary_bh = run_algorithm_backtest(
        df, 'MSTR', initial_qty, peak_date, end_date, algo_bh,
        simple_mode=True
    )
    ...
    _, summary_sd8 = run_algorithm_backtest(
        df, 'MSTR', initial_qty, peak_date, end_date, algo_sd8,
        simple_mode=True
    )
    ...
    print(f"  Interpretation: SD8 {'mitigated' if unrealized_alpha > 0 else 'worsened'} the drawdown by {abs(unrealized_alpha)*100:.2f}%")
        ...
        for ticker in tickers:
            ...
            try:
                results = run_single_asset_retirement_test(
                    ...
                    INITIAL_INVESTMENT, ANNUAL_WITHDRAWAL_RATE
                )
...
def save_results(results: Dict[str, Any], output_dir: Path):
    ...
    with open(md_path, 'w', encoding='utf-8') as f:
        ...
        for ticker, data in results.items():
            ...
            f.write(f"### {ticker}\n\n")
            f.write("| Strategy | Final Value | Total Withdrawn | Survived | Final Purchasing Power |\n")
            f.write("|----------|-------------|-----------------|----------|------------------------|\n")
        ...
        if 'MSTR_unrealized_alpha' in results:
            ...
            f.write(f"**Scenario**: Purchased at peak ${mstr['peak_price']} on {mstr['peak_date']}\n\n")
            f.write("| Metric | Buy-and-Hold | Full SD8 | Alpha |\n")
            f.write("|--------|--------------|----------|-------|\n")
            f.write(f"| Final Value | ${mstr['buy_hold']['final_value']:,.0f} | ${mstr['full_sd8']['final_value']:,.0f} | ${mstr['alpha_value']:,.0f} |\n")
            f.write(f"| Drawdown | {mstr['buy_hold']['drawdown_pct']:.2f}% | {mstr['full_sd8']['drawdown_pct']:.2f}% | {mstr['unrealized_alpha_pct']:.2f}% |\n")
            f.write("\n")
