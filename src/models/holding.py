...
class Transaction:
    ...
    def __post_init__(self):
        ...
        if self.transaction_type not in ('BUY', 'SELL'):
            raise ValueError(f"transaction_type must be 'BUY' or 'SELL', got: {self.transaction_type}")
        if self.shares <= 0:
        ...
        if self.sale_date is not None or self.sale_price is not None:
            ...
            if self.sale_date < self.purchase_date:
                raise ValueError(f"sale_date {self.sale_date} cannot be before purchase_date {self.purchase_date}")
    ...
    def close(self, sale_date: date, sale_price: float) -> None:
        ...
        if sale_date < self.purchase_date:
            raise ValueError(f"sale_date {sale_date} cannot be before purchase_date {self.purchase_date}")
        if sale_price <= 0:
...
class Holding:
    ...
    def market_value(self, current_price: float, current_date: Optional[date] = None) -> float:
        ...
        """
        return sum(
            txn.market_value(current_price, current_date)
            for txn in self.transactions
        )
    ...
    def get_closed_lots(self) -> List[Transaction]:
        ...
        return [
            txn for txn in self.transactions
            if txn.transaction_type == 'BUY' and txn.is_closed
        ]
