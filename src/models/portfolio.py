...
class Portfolio:
    ...
    def buy(
        self, ticker: str, shares: int, purchase_date: date, purchase_price: float, notes: str = ""
    ) -> Transaction:
        """
        ...
        return self.holdings[ticker].add_buy(
            shares=shares, purchase_date=purchase_date, purchase_price=purchase_price, notes=notes
        )
    ...
    def total_shares(self, ticker: Optional[str] = None) -> float:
        ...
        return sum(holding.current_shares() for holding in self.holdings.values())
    ...
    def total_realized_gain_loss(self, ticker: Optional[str] = None) -> float:
        ...
        return sum(holding.realized_gain_loss() for holding in self.holdings.values())
