...
def plot_portfolio_composition(
    ...
    if show_percentage:
        ...
        ax.yaxis.set_major_formatter(
            plt.FuncFormatter(lambda y, _: f"${y/1e6:.2f}M" if y >= 1e6 else f"${y/1e3:.0f}K")
        )

    # Legend
    ax.legend(loc="upper left", framealpha=0.9, fontsize=10, title="Assets", title_fontsize=11)
...
def plot_portfolio_comparison(
    ...
    if best_name:
        ...
        # Plot stacked area
        colors = [get_asset_color(ticker) for ticker in tickers]
        ax2.stackplot(dates, values.T, labels=tickers, colors=colors, alpha=0.8)

        ax2.set_title(
            f"{best_name} Composition (Best Performer: {best_return:+.2f}%)",
            fontsize=14,
            fontweight="bold",
        )
