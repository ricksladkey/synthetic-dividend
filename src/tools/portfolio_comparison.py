...
from src.models.portfolio_simulator import simulate_portfolio, compare_portfolios
from src.visualization.composition_chart import plot_portfolio_composition, plot_portfolio_comparison
...
Examples:
  ...
  python -m src.tools.portfolio_comparison --preset tech_heavy --start 2023-01-01
    ...
    parser.add_argument(
        ...
        help='Use preset allocation'
    )
    
    parser.add_argument(
        '--custom',
        help='Custom allocation (e.g., "NVDA:0.5,VOO:0.5")'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1_000_000,
        help='Initial investment (default: 1,000,000)'
    )
    
    parser.add_argument(
        '--output',
        help='Output chart filename (PNG/PDF)'
    )
    ...
    if args.compare:
        portfolio_configs = [
            (PRESETS[preset]['name'], PRESETS[preset]['allocations'])
    ...
    # Single portfolio mode
        ...
        if not args.no_chart:
            output_file = args.output or f"portfolio_{name.lower().replace(' ', '_')}_{args.start}_{args.end}.png"
