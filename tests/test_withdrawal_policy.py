...
def test_withdrawal_from_bank_balance():
    ...
    assert 10 <= withdrawal_count <= 12, \
        f"Expected ~12 monthly withdrawals, got {withdrawal_count}"
    
    # SD8 generates cash from profit sharing, but may need to sell some shares
    # in early periods before enough cash accumulates
    assert shares_sold_for_withdrawals < 10, \
    ...
    assert abs(actual_per_withdrawal - expected_per_withdrawal) / expected_per_withdrawal < 0.2, \
...
def test_withdrawal_forces_selling_for_buy_and_hold():
    ...
    print(f"Holdings reduction: {initial_shares - final_holdings} shares ({(initial_shares - final_holdings)/initial_shares*100:.1f}%)")
    
    # Buy-and-hold should have forced selling
    assert shares_sold_for_withdrawals > 0, \
        "Buy-and-hold should have been forced to sell shares for withdrawals"
    
    # Final holdings should be less than initial
    assert final_holdings < initial_shares, \
...
def test_simple_mode_no_opportunity_cost():
    ...
    assert results_simple['opportunity_cost'] == 0.0, \
    ...
    assert results_simple['risk_free_gains'] == 0.0, \
    ...
    if results_normal['bank_min'] < 0:
        assert results_normal['opportunity_cost'] > 0, \
