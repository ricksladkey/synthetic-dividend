...
def compare_summaries(
    summary1: Dict[str, Any], summary2: Dict[str, Any], tolerance: float = 1e-10
) -> None:
    ...
    if "daily_values" in summary1 and "daily_values" in summary2:
        ...
        for date_key in daily1:
            if date_key in daily2:
                assert (
                    abs(daily1[date_key] - daily2[date_key]) < tolerance
...
def compare_transactions(txns1: List[Transaction], txns2: List[Transaction]) -> None:
    ...
    for i, (tx1, tx2) in enumerate(zip(txns1, txns2)):
        assert (
            tx1.transaction_date == tx2.transaction_date
        ), f"Tx {i} date: {tx1.transaction_date} != {tx2.transaction_date}"
