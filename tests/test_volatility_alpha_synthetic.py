...
class TestVolatilityAlphaWithSyntheticData(unittest.TestCase):
    ...
    def test_gradual_double_enhanced_vs_ath(self):
        ...
        self.assertEqual(
            len(ath_txns), len(enhanced_txns), "Transaction count should match with no drawdowns"
        )
        self.assertAlmostEqual(
            ath_summary["total_return"],
            enhanced_summary["total_return"],
            places=4,
            msg="Returns should match with no drawdowns",
        )

        # Volatility alpha (Enhanced vs ATH-only) should be ~0
        vol_alpha = enhanced_summary["total_return"] - ath_summary["total_return"]
        self.assertAlmostEqual(vol_alpha, 0, places=4, msg="No volatility alpha without drawdowns")
    ...
    def test_volatile_double_has_positive_volatility_alpha(self):
        ...
        self.assertGreater(
            enhanced_return, ath_return, "Enhanced should outperform ATH-only with volatility"
        )

        # Volatility alpha (Enhanced vs ATH-only) should be positive
        vol_alpha = enhanced_return - ath_return
        self.assertGreater(vol_alpha, 0, "Volatility alpha should be positive")
