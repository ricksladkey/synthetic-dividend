...
class TestBuyHoldWithdrawalSustainability:
    ...
    @pytest.mark.parametrize("withdrawal_rate,expected_survival", [
...
class TestSequenceOfReturnsRisk:
    ...
    def test_good_sequence_bull_then_bear(self, fetcher):
        ...
        print(f"  Year 1 (2019 +31%): ${initial_value:,.0f} → ${summary1['final_value']:,.0f} "
              f"(withdrew ${summary1['total_withdrawn']:,.0f})")
        print(f"  Year 2 (2022 -18%): ${summary1['final_value']:,.0f} → ${final_value:,.0f} "
              f"(withdrew ${summary2['total_withdrawn']:,.0f})")
        print(f"  Final: ${final_value:,.0f}, Total withdrawn: ${total_withdrawn:,.0f}, "
              f"Net: ${net_result:+,.0f}")
    ...
    def test_bad_sequence_bear_then_bull(self, fetcher):
        ...
        print(f"  Year 1 (2022 -18%): ${initial_value:,.0f} → ${summary1['final_value']:,.0f} "
              f"(withdrew ${summary1['total_withdrawn']:,.0f})")
        print(f"  Year 2 (2019 +31%): ${summary1['final_value']:,.0f} → ${final_value:,.0f} "
              f"(withdrew ${summary2['total_withdrawn']:,.0f})")
        print(f"  Final: ${final_value:,.0f}, Total withdrawn: ${total_withdrawn:,.0f}, "
              f"Net: ${net_result:+,.0f}")
