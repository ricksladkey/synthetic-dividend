...
def mock_cpi_prices():
    ...
    return pd.DataFrame(
        {"Close": [100.0, 110.0], "Date": [date(2024, 1, 1), date(2024, 12, 31)]}  # 10% increase
    )
...
def mock_market_prices():
    ...
    return pd.DataFrame(
        {"Close": [100.0, 140.0], "Date": [date(2024, 1, 1), date(2024, 12, 31)]}  # 40% increase
    )
...
@patch("src.data.fetcher.Asset")
def test_calculate_both_adjustments(
    mock_asset_class, sample_summary, mock_cpi_prices, mock_market_prices
):
...
def test_error_handling_insufficient_data(mock_asset_class, sample_summary):
    ...
    mock_asset.get_prices.return_value = pd.DataFrame(
        {"Close": [100.0], "Date": [date(2024, 1, 1)]}  # Only one data point
    )
    mock_asset_class.return_value = mock_asset
