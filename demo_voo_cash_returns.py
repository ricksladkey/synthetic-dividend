...
def compare_cash_strategies(ticker, start_year, end_year, algo_name, withdrawal_rate):
    ...
    _, summary1 = run_retirement_backtest(
        df, ticker, initial_qty, start, end, algo1,
    ...
    _, summary2 = run_retirement_backtest(
        df, ticker, initial_qty, start, end, algo2,
        annual_withdrawal_rate=withdrawal_rate,
        withdrawal_frequency='monthly',
        cpi_adjust=True,
    ...
    print(f"  Holdings: {summary1['holdings']:,.0f} shares = ${summary1['holdings'] * end_price:,.0f}")
    ...
    print(f"  Holdings: {summary2['holdings']:,.0f} shares = ${summary2['holdings'] * end_price:,.0f}")
    ...
    print(f"ACTUAL IMPROVEMENT (gains applied daily and compounded): ${improvement:+,.0f} ({improvement_pct:+.1f}%)")
