...
print(f"{'='*90}")
print(f"Price: ${initial_price:.2f} → ${final_price:.2f} ({(final_price/initial_price - 1)*100:.1f}% gain)")
...
for position_size in position_sizes:
    ...
    for sdn in sdn_values:
        ...
        print(f"  sd{sdn:2d} (δ={bracket_spacing*100:5.2f}%)  "
...
for sdn in sdn_values:
    ...
    for r in sdn_results:
        print(f"  {r['position_size']:6,} shares (Q={r['Q']:6.2f}): "
...
for position_size in position_sizes:
    ...
    for r in size_results:
        ...
        print(f"  sd{r['sdn']:2d}: {r['txns']:4d} txns  "
              f"(txns/n² = {ratio:.2f})")
