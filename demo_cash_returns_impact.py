...
def compare_multi_year_scenarios():
    ...
    for scenario in scenarios:
        ...
        print(
            f"Price: ${start_price:.2f} → ${end_price:.2f} ({(end_price/start_price - 1)*100:+.1f}%)"
        )
        ...
        print(
            f"  Holdings: {summary1['holdings']:,} shares × ${end_price:.2f} = ${summary1['holdings'] * end_price:,.0f}"
        )
        ...
        print(
            f"  Holdings: {summary2['holdings']:,} shares × ${end_price:.2f} = ${summary2['holdings'] * end_price:,.0f}"
        )
        ...
        if summary2["years"] > 0:
            print(
                f"Annualized bank growth from VOO: {(bank_diff / summary1['bank']) / summary2['years'] * 100:.2f}%/year"
                if summary1["bank"] > 0
                else "N/A"
            )
