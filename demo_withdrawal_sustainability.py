...
def demo_withdrawal_scenario(ticker, start_year, end_year, withdrawal_rate, description):
    ...
    _, summary = run_retirement_backtest(
        df, ticker, initial_qty, start, end, algo,
...
def main():
    ...
    _, s1 = run_retirement_backtest(df1, 'SPY', 1000, date(2019, 1, 1), date(2019, 12, 31),
                                     algo, 0.05, 'monthly', simple_mode=True)
    
    df2 = fetcher.get_history('SPY', date(2022, 1, 1), date(2022, 12, 31))
    year2_qty = s1['holdings']
    _, s2 = run_retirement_backtest(df2, 'SPY', year2_qty, date(2022, 1, 1), date(2022, 12, 31),
                                     algo, 0.05, 'monthly', simple_mode=True)
    
    good_net = s2['final_value'] + s1['total_withdrawn'] + s2['total_withdrawn'] - (1000 * df1.iloc[0]['Close'].item())
    ...
    _, s1 = run_retirement_backtest(df1, 'SPY', 1000, date(2022, 1, 1), date(2022, 12, 31),
                                     algo, 0.05, 'monthly', simple_mode=True)
    
    df2 = fetcher.get_history('SPY', date(2019, 1, 1), date(2019, 12, 31))
    year2_qty = s1['holdings']
    _, s2 = run_retirement_backtest(df2, 'SPY', year2_qty, date(2019, 1, 1), date(2019, 12, 31),
                                     algo, 0.05, 'monthly', simple_mode=True)
    
    bad_net = s2['final_value'] + s1['total_withdrawn'] + s2['total_withdrawn'] - (1000 * df1.iloc[0]['Close'].item())
